// send.js - send tokens (integrated with stats)
...
async function sendERC20WithRetries(wallet, token, to, amountHuman, waitConfirm, maxRetries=3, statsModule=null) {
  const contract = new ethers.Contract(token.address, ERC20_ABI, wallet);
  const decimals = await contract.decimals();
  const amountUnits = ethers.parseUnits(String(amountHuman), decimals);
  let attempt = 0;
  if (statsModule) statsModule.inc('attempts', 1);
  while(attempt < maxRetries) {
    attempt++;
    try {
      const tx = await contract.transfer(to, amountUnits);
      console.log(chalk.yellow(`[${now()}] ➜ SENT ${short(tx.hash)}  token:${token.symbol} amount:${amountHuman} to:${to}`));
      if (waitConfirm) {
        const r = await tx.wait(1);
        console.log(chalk.green(`[${now()}] ✅ CONFIRMED ${short(tx.hash)} block:${r.blockNumber} gasUsed:${String(r.gasUsed)}`));
      }
      if (statsModule) statsModule.inc('success', 1);
      return { success:true, txHash: tx.hash };
    } catch(e) {
      const msg = e && e.message ? e.message : String(e);
      console.log(chalk.red(`[${now()}] ⚠ Error attempt ${attempt}: ${msg}`));
      if (attempt >= maxRetries) {
        if (statsModule) statsModule.inc('failed', 1);
...
module.exports.runSendMenu = async function({ provider, wallet, ethers:ethersLib, tokens, stats } = {}) {
  ...
  while(true) {
    ...
    const selNum = Number(await askInput('Pilih menu (masukkan nomor):','')) - 1;
    if (isNaN(selNum) || selNum < 0 || selNum >= choices.length) { console.log('Pilihan tidak valid'); continue; }
    const isBack = selNum === (choices.length-1);
    if (isBack) return;
    const isSendAll = selNum === (choices.length-2);
    let destType;
    if (isSendAll) {
      ...
      const d = await askNumbered(['Send to Random Address','Send to Manual Address','Back'], `Tujuan untuk ${tokens[selNum].symbol}:`);
      if (d === 2) continue;
      destType = d===0 ? 'random' : 'manual';
    }
    let manualTo = null;
    ...
    if (isSendAll) {
      for (const token of tokens) {
        ...
        while(sendCount===0 ? true : sentForToken < sendCount) {
          ...
          if (BigInt(balInfo.bal) < BigInt(unitAmount)) { console.log(chalk.yellow(`[${now()}] Insufficient ${token.symbol} balance (${balInfo.human}), skip token.`)); break; }
          const to = destType==='random' ? ethers.Wallet.createRandom().address : manualTo;
          attempts++;
          const res = await sendERC20WithRetries(wallet, token, to, amount, waitConfirm, 3, stats);
          progress.increment();
          if (res.success) { success++; if (res.txHash) console.log(chalk.cyan(`[${now()}] TX: ${process.env.EXPLORER_BASE || ''}/tx/${res.txHash}`)); sentForToken++; }
          else { failed++; }
    ...
    console.log('========================\n');
    if (stats) { stats.inc('attempts', attempts); stats.inc('success', success); stats.inc('failed', failed); }
  }
};
