// faucet_rpc.js (tempo_fundAddress caller) integrated with stats
...
module.exports.runInteractive = async function({ provider, wallet, ethers:ethersPassed, stats } = {}) {
  if (!process.env.RPC_URL) { console.log(chalk.red('Set RPC_URL in .env')); return; }
  ...
  let addresses = [];
  if (selIdx === 0) {
    if (!process.env.PRIVATE_KEY) { console.log(chalk.red('PRIVATE_KEY missing')); return; }
    addresses = [(new ethers.Wallet(process.env.PRIVATE_KEY)).address];
  } else if (selIdx === 1) {
    const manual = (await rlQuestion('Masukkan address tujuan: ')).trim();
    if (!ethers.isAddress(manual)) { console.log(chalk.red('Address invalid')); return; }
    addresses = [manual];
  } else if (selIdx === 2) {
    const filePath = (await rlQuestion('Path file (one address per line): ')).trim() || './addresses.txt';
    try { const txt = fs.readFileSync(filePath,'utf8'); addresses = txt.split(/\r?\n/).map(s=>s.trim()).filter(Boolean); } catch(e){ console.log(chalk.red('Read file failed')); return; }
  }
  ...
  for (const addr of addresses) {
    for (let i=0;i<perAddressCount;i++) {
      ...
      while(attempt < maxRetries && !ok) {
        attempt++;
        try {
          const res = await providerLocal.send('tempo_fundAddress', [addr]);
          ok = true;
          totalSuccess++;
          console.log(chalk.green(`OK. result: ${JSON.stringify(res).slice(0,200)}`));
          ...
          console.log(chalk.yellow(`Error attempt ${attempt}: ${lastRes}`));
  ...
  if (stats) { stats.inc('attempts', totalAttempts); stats.inc('success', totalSuccess); stats.inc('failed', totalFailed); }
};
if (require.main === module) module.exports.runInteractive().catch(e=>{ console.error('Fatal:', e && e.stack ? e.stack : e); process.exit(1); });
